...
REVIEW_IDS_PATH = "../../DATA/stage1_outputs/final_best_logit/REVIEW_ids_for_stage2.parquet"
IN_STAGE2_PATH  = "../../DATA/dataset/test_stage2"
STAGE1_PATH     = "../../DATA/dataset/TEST_stage1"
OUT_STAGE2_PATH = "../../DATA/dataset/TEST_stage2"
...
def build_test_stage2_from_review_ids(
    in_stage2_path=IN_STAGE2_PATH,
    stage1_path=STAGE1_PATH,
    review_ids_path=REVIEW_IDS_PATH,
    out_stage2_path=OUT_STAGE2_PATH,
):
    # 1) REVIEW ids
    review_ids = pd.read_parquet(review_ids_path)[IDCOL].astype("int64").unique()

    # 2) load base stage2 raw (FULL needed for history)
    ...
    # ---------- client_error_last3 (요청 그대로: fraud shift 합) ----------
    ...
    # 3) filter to REVIEW ids + keep only stage2 requested cols (+ id,fraud)
    df_out = df[df[IDCOL].isin(review_ids)].copy()

    _require_cols(df_out, [IDCOL, LABEL] + STAGE2_COLS)
    df_out = df_out[[IDCOL, LABEL] + STAGE2_COLS].copy()

    # 4) load stage1 features and merge
    st1 = pd.read_parquet(stage1_path)
    _require_cols(st1, [IDCOL, LABEL])
    st1[IDCOL] = pd.to_numeric(st1[IDCOL], errors="coerce").astype("int64")

    # stage1에서 id,fraud 제외 전부 feature로 붙임
    stage1_feat_cols = [c for c in st1.columns if c not in [IDCOL, LABEL]]
    st1_small = st1[st1[IDCOL].isin(review_ids)][[IDCOL] + stage1_feat_cols].copy()

    # merge (left=stage2 review rows)
...
if __name__ == "__main__":
    build_test_stage2_from_review_ids()